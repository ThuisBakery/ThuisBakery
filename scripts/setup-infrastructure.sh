...
# ── 2 ─────────────────────────────────────────────────────────────────────
stage "Confirm the repo is org-owned"
say "Vercel will not let a repo COLLABORATOR create a project from a repo owned"
say "by a personal account — only its Owner. An organization has no such rule:"
say "any member with repo access can import."
say ""
say "This was done on 2026-09-22: the ThuisBakeryTech personal account was"
say "converted to an organization and renamed ThuisBakery, with DParfitt as"
say "owner. The repo is now ThuisBakery/ThuisBakery. Nothing to redo — an"
say "organization cannot be converted back, so this stage only verifies."
step "Check you still have admin:"
note "  gh api repos/ThuisBakery/ThuisBakery --jq .permissions"
...
if confirm "Does that report admin: true?"; then
  say "Good — the import in the next stage will work."
else
  SKIPPED+=("repo admin on ThuisBakery/ThuisBakery — Vercel import needs it")
  warn "Without it, stage 3 will not find the repo in Vercel's import list."
fi
...
step "Import ThuisBakery/ThuisBakery."
...
note "on the ThuisBakery organization, then push an empty commit."
